function [sPb] = spectralPb(mPb, orig_sz, outFile, nvec,BIN_SEGMENT)
...
if (nargin<4) || isempty(nvec), nvec = 17; end
...
system(sprintf('%s -image %s -smatrixfile %s -readlattice true -latticefile %s -dthresh 5', BIN_SEGMENT,imgFile, simFile, latFile));
delete(latFile);
delete(imgFile);

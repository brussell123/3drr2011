...
% Compute angles between 3D points:
C = inv(K*K');
c12 = -2*u1'*C*u2/sqrt((u1'*C*u1)*(u2'*C*u2));
c13 = -2*u1'*C*u3/sqrt((u1'*C*u1)*(u3'*C*u3));
c23 = -2*u2'*C*u3/sqrt((u2'*C*u2)*(u3'*C*u3));

% Compute distances between 3D points:
d12 = sqrt(sum((X1-X2).^2));
d13 = sqrt(sum((X1-X3).^2));
d23 = sqrt(sum((X2-X3).^2));

a = d23; b = d13; c = d12;
alpha = acos(-0.5*c23); beta = acos(-0.5*c13); gamma = acos(-0.5*c12);
